...
inline void* MemAllocInit(size_t count, size_t sz) { return ::calloc(count, sz); }
...
public:
	...
	static jmethodID	m_createFormulaException;
	static jmethodID	m_createExceptionWithoutStackTrace;
	...
	static jobject createException(JNIEnv* env, STATUS status, jstring msg) {
		return env->CallStaticObjectMethod(jniClass, m_createException, status, msg);
	}
	static jobject createFormulaException(JNIEnv *env, STATUS status, jstring msg, STATUS compileError, jstring formulaMsg, WORD errorLine, WORD errorColumn, WORD errorOffset, WORD errorLength) {
		return env->CallStaticObjectMethod(jniClass, m_createFormulaException, status, msg, (jint)compileError, formulaMsg, (jint)errorLine, (jint)errorColumn, (jint)errorOffset, (jint)errorLength);
	}
	static jobject createExceptionWithoutStackTrace(JNIEnv *env, STATUS status, jstring msg) {
		return env->CallStaticObjectMethod(jniClass, m_createExceptionWithoutStackTrace, status, msg);
	}
